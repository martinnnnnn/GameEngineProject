	...
	XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, deviceContext, "Data/Font/FontData.txt", "Data/Texture/font.tga");
	if (!result)
	{
...
bool TextHandler::Render(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX orthoMatrix)
{
	bool result;
...
bool TextHandler::RenderSentence(ID3D11DeviceContext* deviceContext, SentenceType* sentence, XMMATRIX worldMatrix,
...
bool TextHandler::SetMousePosition(int mouseX, int mouseY, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char mouseString[16];
	bool result;
...
bool TextHandler::SetText(const char* text, ID3D11DeviceContext* deviceContext)
{
	bool result;
	char fpsString[16];
	strcpy_s(fpsString, text);
	result = UpdateSentence(m_sentence1, fpsString, 20, 20, 255, 255, 255, deviceContext);
	if (!result)
	{
		return false;
	}
